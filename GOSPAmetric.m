...
for ix = 1:Nx
    for iy = 1:Ny
        ...
        %gwd = GaussianWassersteinDistance(X.xshape(1:2,1,ix)+X.x(1:2,ix),X.Xshape(:,:,1,ix),Y.xshape(1:2,1,iy)+Y.x(1:2,iy),Y.Xshape(:,:,1,iy));
        %gwd = GaussianWassersteinDistance(X.x(1:2,ix),X.X(:,:,ix),Y.xshape(1:2,iy),Y.Xshape(:,:,iy));
