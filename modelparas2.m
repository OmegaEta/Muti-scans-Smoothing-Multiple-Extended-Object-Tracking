load('Scenario1_R_2.mat')
load('Scenario1_R_Truth_e_2.mat')

groundTruth=groundTruth_e;
...
xstart(:,1) = [-196 132 0.2 -5.9];
xstart(:,2) = [-190 0 3.8 0];
xstart(:,3) = [-196 -132 0.2 5.9];

%Birth model
d = 2;
model.birth.w = 0.25*ones(nbirths,1);
model.birth.GGIW = repmat(struct('a',100,'b',10,'m',[],'P',diag([4,4,4,4]),'v',22.5,'V',72.5*eye(2)),[nbirths,1]);
for i = 1:nbirths
...
model.threshold_w = 0.1;   %1e-2 weight of global hypothesis (multi-Bernoulli)
