...
for i = 1:n_tt
    ...
    for j = 1:length(meas_track{i})
        for k = 1:size(meas_track{i}{j},1)
            ...
            GGIWpre=MBM.track{i}(j).Bern.GGIW(end);
...
if n_tt_upd > n_tt
    for i = 1:n_tt_upd - n_tt
        ...
        if any(in_gate)
            [Bern,lik,GGIWpre] = detectionPPP(PPP.w(in_gate),PPP.GGIW(in_gate),W(:,meas_track{n_tt+i}{1}),model);
            Bern.t_birth = time;
            Bern.t_death = time;
            Bern.w_death = 1;
        else
            GGIWpre=struct('a',0,'b',1,'v',0,'P',zeros(2,2),'V',zeros(2,2));
            Bern.r = 0; 
            Bern.GGIW = struct('a',0,'b',1,'v',0,'P',zeros(2,2),'V',zeros(2,2)); 
            lik = [];
        end
        Bern.GGIWpre=GGIWpre;
